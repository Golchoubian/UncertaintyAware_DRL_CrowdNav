...
class Config(object):
    ...
    # 'none': no prediction
    sim.predict_method = 'inferred' # Also adjust the uncertainty-aware args in the argument.py file!
    sim.predict_network = 'VanillaLSTM' # VanillaLSTM or CollisionGrid
    sim.uncertainty_aware = args.uncertainty_aware
    
    # render the simulation during training or not
    # during test time make sure to keep this False 
    # (for skipping automatic reset in dummy_vec_env during test time)
    sim.render = False 
    ...
    # = True only if we are using a network for human trajectory prediction 
