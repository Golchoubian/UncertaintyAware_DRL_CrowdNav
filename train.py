...
def main():
    ...
    shutil.copy('move_plan/configs/config.py', save_config_dir)
    shutil.copy('move_plan/configs/__init__.py', save_config_dir)
    ...
    envs = make_vec_envs(env_name, algo_args.seed, algo_args.num_processes,
                         algo_args.gamma, '', device, False, config=env_config,
                         ax=ax, fig=fig, pretext_wrapper=config.env.use_wrapper)
    ...
    for j in range(num_updates):
        ...
        if algo_args.use_linear_lr_decay and j>algo_args.lr_decay_start_epoch:
            start_j = algo_args.lr_decay_start_epoch
            network_utils.update_linear_schedule(
                agent.optimizer, (j-start_j), (num_updates-start_j),
        ...
        for step in range(algo_args.num_steps):
            ...
            # if we use prediction, send predictions to env for rendering
            if env_name == 'PedSimPred-v0' and env_config.env.use_wrapper:
                # [nenv, max_human_num, 2*(pred_steps+1)] -> [nenv, max_human_num, 2*pred_steps]
                if config.sim.uncertainty_aware:
                    out_pred = rollouts_obs['pred_pos'][:, :, 6:].to('cpu')
                else:
                    # [nenv, max_human_num, 2*pred_steps]
                    out_pred = rollouts_obs['spatial_edges'][:, :, 2:].to('cpu')
                next_obs = rollouts_obs['ped_pos'] # [nenv, obs_len, max_human_num, 5]
                next_mask = rollouts_obs['ped_mask'] # [nenv, obs_len, max_human_num]

              
                # Combining the three above tensors, to send it as one tensor to the environment
                # throught talk2Env, we can do so since our pred_len and obs_len are equal
                if config.sim.uncertainty_aware:
                    data = torch.zeros(envs.num_envs, config.sim.pred_len, 
                                       config.sim.max_human_num, 12)
                    data[:,:,:,:6] = out_pred.reshape(envs.num_envs, config.sim.max_human_num, 
                                                      config.sim.pred_len, 6).permute(0, 2, 1, 3)
                    data[:,:,:,6:11] = next_obs
                    data[:,:,:,11] = next_mask
                else:
                    data = torch.zeros(envs.num_envs, config.sim.pred_len, 
                                       config.sim.max_human_num, 8)
                    data[:,:,:,:2] = out_pred.reshape(envs.num_envs, config.sim.max_human_num,
                                                      config.sim.pred_len, 2).permute(0, 2, 1, 3)
                    data[:,:,:,2:7] = next_obs
                    data[:,:,:,7] = next_mask

                # Passsing the predicted traj of peds calcualated in VecPretextNormalize
                # to the environment for rendering
                ack = envs.talk2Env(data)
