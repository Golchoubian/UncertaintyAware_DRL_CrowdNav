...
class SpatialEdgeSelfAttn(nn.Module):
    ...
    def __init__(self, args):
        ...
        if args.env_name in ['CrowdSimPred-v0', 'CrowdSimPredRealGST-v0']:
            self.input_size = 12
        elif args.env_name in  ['CrowdSimVarNum-v0', 'PedSim-v0']: # No prediction
            self.input_size = 2
        elif args.env_name == 'PedSimPred-v0':  # With prediction
            # pred 6 steps ahead + the curret step: 2*(6+1) = 14
            if args.uncertainty_aware: 
                # also store the covarince matrices of the pedestrians predicted position
                # over the predcition horizon
                self.input_size = 42 # 6*(6+1) = 42
            else: 
                self.input_size = 14
        else:
