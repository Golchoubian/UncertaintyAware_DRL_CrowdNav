...
class SpatialEdgeSelfAttn(nn.Module):
    ...
    def __init__(self, args, is_veh=False):
        ...
        if args.env_name in ['CrowdSimPred-v0', 'CrowdSimPredRealGST-v0']:
            ...
            self.input_size = 2  # 4
        elif args.env_name == 'PedSimPred-v0' and is_veh==False:  # self attention for humans
            # pred 6 steps ahead + the curret step: 2*(6+1) = 14
            self.input_size = 14
        elif args.env_name == 'PedSimPred-v0' and is_veh==True:  # self attention for vehicles
            # for vehicles we have no prediction, so we only have the current sate of them 
            # with respect to the robot as the spatial edge
            self.input_size = 2
        elif args.env_name == 'PedSim-v0':  # for both humans and vehicles
            self.input_size = 2
        else:
...
class EdgeAttention_M(nn.Module):
    ...
    def att_func(self, temporal_embed, spatial_embed, h_spatials, attn_mask=None):
        ...
        # reshape back
        weighted_value = weighted_value.squeeze(-1).view(seq_len, nenv,
                                                         self.agent_num, h_size)  # [seq_len, 12, 6 or 1, 256]
    ...
    def forward(self, h_temporal, h_spatials, each_seq_len):
        ...
        Forward pass for the model
        params:
        h_temporal : Hidden state of the temporal edgeRNN   (Robot information)
        h_spatials : Hidden states of all spatial edgeRNNs connected to the node. (RH spatial edge information)
        each_seq_len:
            if self.args.sort_humans is True, the true length of the sequence. Should be the number of detected humans
            else, it is the mask itself
        '''
        ...
        for i in range(self.num_attention_head):
            ...
            if self.args.sort_humans:
                attn_mask = self.create_attn_mask(each_seq_len, seq_len, nenv,
                                                  max_human_num)  # [seq_len*nenv, 1, max_human_num]
...
class EndRNN(RNNBase):
    ...
    def __init__(self, args):
        ...
        # Linear layer to embed input
        self.encoder_linear = nn.Linear(256, self.embedding_size) # embed robot state
        ...
        # Linear layer to embed vehicle attention module output
        self.edge_attention_embed_veh = nn.Linear(self.edge_rnn_size, self.embedding_size)

        # Output linear layer
        self.output_linear = nn.Linear(self.rnn_size, self.output_size)

    def forward(self, robot_s, h_spatial_other, h_spatial_veh, h, masks):
        '''
        Forward pass for the model
        params:
        pos : input position
        ...
        h_spatial_veh : output of the attention module for vehicles
        ...
        h_edges_embedded_veh = self.relu(self.edge_attention_embed_veh(h_spatial_veh))

        concat_encoded = torch.cat((encoded_input, h_edges_embedded, h_edges_embedded_veh), -1)  
...
class double_selfAttn(nn.Module):
    """
    ...
    def __init__(self, obs_space_dict, args, infer=False):
        ...
        infer : Training or test time (True at test time)
        """
        super(double_selfAttn, self).__init__()
        self.infer = infer
        self.is_recurrent = True
        self.args = args

        self.human_num = obs_space_dict['spatial_edges'].shape[0]
        self.vehicle_num = obs_space_dict['spatial_edges_veh'].shape[0]
        ...
        self.attn_veh = EdgeAttention_M(args)
        ...
        # for vehicle spatial edges
        if self.args.use_self_attn_veh:
            self.spatial_attn_veh = SpatialEdgeSelfAttn(args, is_veh=True)
            self.spatial_linear_veh = nn.Sequential(init_(nn.Linear(512, 256)), nn.ReLU())
        else:
            self.spatial_linear_veh = nn.Sequential(init_(nn.Linear(obs_space_dict['spatial_edges_veh'].shape[1], 128)), nn.ReLU(),
                                                init_(nn.Linear(128, 256)), nn.ReLU())
    ...
    def forward(self, inputs, rnn_hxs, masks, infer=False):
        ...
        spatial_edges_veh = reshapeT(inputs['spatial_edges_veh'], seq_length, nenv)
        ...
        if self.args.sort_humans:
            ...
            detected_vehicle_num = inputs['detected_vehicle_num'].squeeze(-1).cpu().int()
        else:
        ...
        if self.args.sort_humans:
            ...
            if self.args.use_self_attn_veh:
                spatial_attn_out_veh = self.spatial_attn_veh(spatial_edges_veh, detected_vehicle_num).view(
                    seq_length, nenv, self.vehicle_num, -1)
            else:
                spatial_attn_out_veh = spatial_edges_veh
            output_spatial_veh = self.spatial_linear_veh(spatial_attn_out_veh)

            # robot-vehicle attention
            hidden_attn_weighted_veh, _ = self.attn_veh(robot_states, output_spatial_veh, detected_vehicle_num)
        else:
            # human-human attention
            if self.args.use_self_attn:
        ...
        outputs, h_nodes \
            = self.humanNodeRNN(robot_states, hidden_attn_weighted, hidden_attn_weighted_veh, hidden_states_node_RNNs, masks)
