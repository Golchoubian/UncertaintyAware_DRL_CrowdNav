...
def make_env(env_id, seed, rank, log_dir, allow_early_resets, config=None,
              envNum=1, ax=None, fig=None, test_case=-1):
    def _thunk():
...
def make_vec_envs(env_name,
                  ...
                  config=None,
                  ax=None, fig=None, test_case=-1, wrap_pytorch=True, pretext_wrapper=False):
    envs = [
        make_env(env_name, seed, i, log_dir, allow_early_resets, config=config,
                 envNum=num_processes, ax=ax, fig=fig, test_case=test_case)
    ...
    if len(envs) > 1:
        ...
        envs = DummyVecEnv(envs, test, config.sim.render)
    ...
    if pretext_wrapper:
        if gamma is None:
            ...
            envs = VecPretextNormalize(envs, gamma=gamma, ob=False, ret=False, config=config,
                                        test=test)
...
class TimeLimitMask(gym.Wrapper):
    def step(self, action, pred):
        obs, rew, done, info = self.env.step(action, pred)
